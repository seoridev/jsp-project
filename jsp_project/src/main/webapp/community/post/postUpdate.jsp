...
<%
    ...
    if (!isWriter && !manager) {
        response.sendRedirect(request.getContextPath() + "/community/post/postDetail.jsp?postId=" + postId + "&error=updateDenied");
        return;
    }
%>
<!DOCTYPE html>
            ...
            <div class="cafe-box">
                ...
                <nav class="cafe-menu-list" aria-label="카페 메뉴">
                    ...
                    <% for (CafeBoardDTO cafeBoard : boards) { %>
                        <a class="cafe-menu-item <%= cafeBoard.getBoardId() == post.getBoardId() ? "active" : "" %>" href="<%= contextPath %>/community/post/postList.jsp?cafeId=<%= cafeId %>&boardId=<%= cafeBoard.getBoardId() %>">
        ...
        <section class="cafe-main">
            <section class="cafe-write-panel">
                <div class="cafe-write-head">
                    <p class="breadcrumb">
                        <a href="<%= contextPath %>/community/cafe/cafeDetail.jsp?cafeId=<%= cafeId %>"><%= escapeHtml(post.getCafeName()) %></a>
                        <span>&gt;</span>
                        <a href="<%= contextPath %>/community/post/postList.jsp?cafeId=<%= cafeId %>&boardId=<%= post.getBoardId() %>"><%= escapeHtml(post.getBoardName()) %></a>
                    </p>
                    <h1>게시글 수정</h1>
                </div>
                ...
                <% } %>
                <form class="cafe-write-form" action="<%= contextPath %>/community/post/postUpdateProcess.jsp" method="post">
                    ...
                    <div class="write-actions">
                        <a class="button btn-sub" href="<%= contextPath %>/community/post/postDetail.jsp?postId=<%= postId %>">취소</a>
