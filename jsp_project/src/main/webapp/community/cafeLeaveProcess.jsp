...
<%
    ...
    if (cafeId <= 0 || member == null || !"ACTIVE".equals(member.getStatus())) {
        response.sendRedirect(request.getContextPath() + "/community/cafeDetail.jsp?cafeId=" + cafeId + "&error=leaveFail");
        return;
    }

    if ("OWNER".equals(member.getRole())) {
        response.sendRedirect(request.getContextPath() + "/community/cafeDetail.jsp?cafeId=" + cafeId + "&error=ownerLeaveDenied");
        return;
    }
