...
<%
    ...
    if (!valid) {
        response.sendRedirect(request.getContextPath() + "/community/cafeBoardManage.jsp?cafeId=" + cafeId + "&error=hideFail");
        return;
    }

    if (boardDao.hasActivePosts(boardId)) {
        response.sendRedirect(request.getContextPath() + "/community/cafeBoardManage.jsp?cafeId=" + cafeId + "&error=hasPosts");
        return;
    }
